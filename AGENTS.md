...
- **Adding HTTP handlers without `#[utoipa::path]` annotations.** Every handler that returns JSON must have utoipa annotations and be registered in `ApiDoc` (`service/src/rest.rs`). Without this, `just codegen` won't generate TypeScript types and the frontend will use hand-written types that drift from the backend. `just lint-patterns` enforces this — suppress with `// lint-patterns:allow-no-utoipa` plus a tracking issue.
