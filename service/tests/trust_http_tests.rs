...
struct StubListMyDenouncementsReturnsError;

#[async_trait]
impl TrustRepo for StubListMyDenouncementsReturnsError {
    async fn list_denouncements_by_with_username(
        &self,
        _accuser_id: Uuid,
    ) -> Result<Vec<DenouncementWithUsername>, TrustRepoError> {
    ...
    async fn get_or_create_influence(
        ...
        unimplemented!("StubListMyDenouncementsReturnsError: must not be called in this test")
    }
...
#[shared_runtime_test]
async fn list_my_denouncements_returns_500_when_repo_fails() {
    let db = isolated_db().await;
    let (_, keys, _) = signup_and_get_account("listdenounceerr", db.pool()).await;

    let app = TestAppBuilder::new()
        .with_identity_pool(db.pool().clone())
        .with_stub_trust_repo(Arc::new(StubListMyDenouncementsReturnsError))
        .build();
    ...
    assert_eq!(
        response.status(),
        StatusCode::INTERNAL_SERVER_ERROR,
        "list_my_denouncements_handler must return 500 when list_denouncements_by_with_username fails"
    );
}
