...
/// simulating a storage failure on the `GET /trust/scores/me` path.
...
///
/// A stub repo injects the failure to confirm the error path in the handler
/// fires correctly and the client receives a 500 response.
#[shared_runtime_test]
async fn scores_me_returns_500_when_get_all_scores_fails() {
    let db = isolated_db().await;
    let (_, keys, _) = signup_and_get_account("scoresme500", db.pool()).await;
