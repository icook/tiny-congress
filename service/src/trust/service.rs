...
mod tests {
    ...
    async fn denounce_proceeds_when_daily_count_is_one_below_quota() {
        ...
        // (one action slot still available). The guard is `daily_count >= daily_quota`,
        // so this must NOT fire.
        // This is the complement of denounce_returns_quota_exceeded_when_daily_limit_reached,
        // which tests the upper boundary (daily_count == quota → blocked). Together they pin
        // the `>=` comparison: if it were changed to `>`, the upper-boundary test would catch
        // it; if a +1 offset were added (e.g. `daily_count + 1 >= quota`), this lower-boundary
        // test would catch it.
        let captured = Arc::new(Mutex::new(None::<serde_json::Value>));
        let svc = DefaultTrustService::new(
            Arc::new(
