...
mod tests {
    ...
    /// and whose `count_active_trust_endorsements_by` always returns an error.
    /// Used to verify that `DefaultTrustService::endorse` propagates the error
    /// rather than silently proceeding to enqueue.
    struct ErrorCountActiveEndorsementsRepo;

    #[async_trait]
    impl ReputationRepo for ErrorCountActiveEndorsementsRepo {
        async fn has_endorsement(&self, _: Uuid, _: &str) -> Result<bool, EndorsementRepoError> {
            Ok(false) // non-verifier — forces the slot-count branch
        }
        async fn count_active_trust_endorsements_by(
            &self,
            _: Uuid,
    ...
    #[tokio::test]
    async fn endorse_propagates_endorsement_repo_error_when_slot_count_fails() {
        // When a non-verifier caller triggers the slot-count branch and
        // `count_active_trust_endorsements_by` returns an error, `endorse` must
        // propagate it as `TrustServiceError::EndorsementRepo` rather than
        // silently treating the slot count as zero and proceeding to enqueue.
        //
        // Uses `CapturingEnqueueRepo` as the TrustRepo stub so all upstream trust-repo
        // guards (quota, denouncement conflict) pass, ensuring the path reaches
        // `count_active_trust_endorsements_by` before the error is returned.
        let captured = Arc::new(Mutex::new(None));
        let svc = DefaultTrustService::new(
            Arc::new(CapturingEnqueueRepo {
                captured: captured.clone(),
        ...
        assert!(
            matches!(err, TrustServiceError::EndorsementRepo(_)),
            "expected EndorsementRepo error when slot count fails, got: {err}"
        );
        // Payload must NOT have been captured — the error fires before enqueue_action.
        assert!(
            captured.lock().unwrap().is_none(),
            "enqueue_action must not be called when slot count fails"
        );
    }
}
