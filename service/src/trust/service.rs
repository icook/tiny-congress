...
mod tests {
    ...
    // ─── endorsement repo error propagation (slot count) ─────────────────────

    /// A [`ReputationRepo`] stub whose `has_endorsement` returns `Ok(false)` (non-verifier)
    /// but `count_active_trust_endorsements_by` always returns an error.
    /// Used to verify that `DefaultTrustService::endorse` propagates the error from the
    /// slot-count step rather than silently proceeding to enqueue.
    struct ErrorCountActiveEndorsementsRepo;

    #[async_trait]
    impl ReputationRepo for ErrorCountActiveEndorsementsRepo {
        async fn has_endorsement(&self, _: Uuid, _: &str) -> Result<bool, EndorsementRepoError> {
            Ok(false) // not a verifier — triggers the slot-count branch
        }
        async fn count_active_trust_endorsements_by(
            &self,
            _: Uuid,
    ...
    #[tokio::test]
    async fn endorse_propagates_endorsement_repo_error_from_count_active_endorsements() {
        // When the endorser is not a verifier and `count_active_trust_endorsements_by`
        // returns an error, `endorse` must propagate it as
        // `TrustServiceError::EndorsementRepo` rather than silently proceeding to enqueue.
        //
        // Uses `CapturingEnqueueRepo` as the TrustRepo stub so all upstream guards
        // (quota, denouncement conflict) pass, ensuring the path reaches the slot-count
        // call before the error is returned.
        let captured = Arc::new(Mutex::new(None));
        let svc = DefaultTrustService::new(
            Arc::new(CapturingEnqueueRepo {
                captured: captured.clone(),
