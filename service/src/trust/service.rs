...
mod tests {
    ...
    // ─── denounce count_daily_actions repo-error propagation test ─────────────

    /// Stub [`TrustRepo`] that passes `has_active_denouncement` but fails
    /// `count_daily_actions` with a database error — used to verify that
    /// `denounce` propagates the error rather than silently swallowing it.
    struct FailingCountDailyActionsForDenounceRepo;

    #[async_trait]
    impl TrustRepo for FailingCountDailyActionsForDenounceRepo {
        async fn has_active_denouncement(&self, _: Uuid, _: Uuid) -> Result<bool, TrustRepoError> {
            Ok(false)
        }
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
    ...
    async fn denounce_propagates_repo_error_from_count_daily_actions() {
        // `count_daily_actions` is the second repo call in `denounce` (after
        // `has_active_denouncement`). If the database fails there, the service
        // must surface the error rather than silently succeeding or panicking.
        // `FailingCountDailyActionsForDenounceRepo` panics on every other method,
        // so a missing `?` would cause the test to fail loudly.
        let svc = DefaultTrustService::new(
            Arc::new(FailingCountDailyActionsForDenounceRepo),
            Arc::new(PanicReputationRepo),
        );
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
