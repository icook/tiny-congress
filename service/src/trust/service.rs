...
mod tests {
    ...
    // ─── Stub for repo-backed guard tests ─────────────────────────────────────
    //
    // Returns configurable responses for the two repo calls reached before the
    // DenouncementConflict check fires in `endorse`. All other methods panic so
    // a missing guard is caught immediately.

    struct ActiveDenouncement;

    #[async_trait]
    impl TrustRepo for ActiveDenouncement {
        async fn get_or_create_influence(
            &self,
            _: Uuid,
        ) -> Result<InfluenceRecord, TrustRepoError> {
        ...
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
            Ok(0) // well below quota — lets execution reach has_active_denouncement
        }
        async fn get_action(&self, _: Uuid) -> Result<ActionRecord, TrustRepoError> {
            unimplemented!()
        }
        ...
        async fn has_active_denouncement(&self, _: Uuid, _: Uuid) -> Result<bool, TrustRepoError> {
            Ok(true) // endorser has already denounced the subject
        }
        async fn create_invite(
            &self,
            _: Uuid,
    ...
    #[tokio::test]
    async fn endorse_returns_denouncement_conflict_when_endorser_has_active_denouncement() {
        // ADR-024: endorsement and denouncement are mutually exclusive.
        // If the endorser already has an active denouncement against the subject,
        // endorse must reject with DenouncementConflict rather than queuing.
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
        let service =
            DefaultTrustService::new(Arc::new(ActiveDenouncement), Arc::new(PanicReputationRepo));
        let err = service.endorse(a, b, 0.5, None).await.unwrap_err();
        assert!(
