...
mod tests {
    ...
    /// Stub [`TrustRepo`] that passes all pre-enqueue guards in
    /// [`DefaultTrustService::denounce`] (denouncement conflict check, daily
    /// quota check, and slot limit check) but returns a database error from
    /// `enqueue_action`. Used to verify the service propagates the error
    ...
    async fn denounce_propagates_repo_error_from_enqueue_action() {
        ...
        // (self-action, reason validation, denouncement conflict check, daily
        // quota check, and slot limit check). If the database fails at the
        // enqueue step, the service must surface the error rather than silently
        // succeeding. `FailingEnqueueForDenounceRepo` panics on all other
        // methods, so a stray call would cause the test to fail loudly.
        let svc = DefaultTrustService::new(
