...
mod tests {
    ...
    // ─── endorse repo-error propagation test ─────────────────────────────────

    /// Stub [`TrustRepo`] that passes the daily quota guard and then fails
    /// `has_active_denouncement` with a database error — used to verify that
    /// `endorse` propagates the error rather than silently treating the caller
    /// as having no denouncement conflict and proceeding to enqueue.
    struct PassesQuotaFailsHasActiveDenouncement;

    #[async_trait]
    impl TrustRepo for PassesQuotaFailsHasActiveDenouncement {
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
            Ok(0) // below quota — lets the endorse path reach has_active_denouncement
        }
        async fn has_active_denouncement(&self, _: Uuid, _: Uuid) -> Result<bool, TrustRepoError> {
            Err(TrustRepoError::Database(sqlx::Error::RowNotFound))
        }
    ...
    async fn endorse_propagates_repo_error_from_has_active_denouncement() {
        // `has_active_denouncement` is called in `endorse` (after the quota check) to
        // prevent endorsing someone the caller has already denounced (ADR-024). If the
        // database fails there, the service must surface the error rather than silently
        // treating it as "no conflict" and proceeding to enqueue.
        //
        // `PassesQuotaFailsHasActiveDenouncement` returns Ok(0) from count_daily_actions
        // so the quota guard passes, then returns Err from has_active_denouncement. It
        // panics on enqueue_action, so a missing `?` would cause the test to panic.
        let svc = DefaultTrustService::new(
            Arc::new(PassesQuotaFailsHasActiveDenouncement),
            Arc::new(PanicReputationRepo),
        );
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
