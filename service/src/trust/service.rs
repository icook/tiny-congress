...
mod tests {
    ...
    /// Stub [`TrustRepo`] that returns no active denouncement and is under the
    /// daily quota, but fails `count_total_denouncements_by` — used to verify
    /// that `denounce` propagates the error rather than silently swallowing it.
    struct FailingCountTotalDenouncementsRepo;

    #[async_trait]
    impl TrustRepo for FailingCountTotalDenouncementsRepo {
        async fn has_active_denouncement(&self, _: Uuid, _: Uuid) -> Result<bool, TrustRepoError> {
            Ok(false)
        }
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
    ...
    async fn denounce_propagates_repo_error_from_count_total_denouncements_by() {
        ...
        // and `count_daily_actions` pass. A database failure here must be surfaced
        // as `TrustServiceError::Repo` rather than silently swallowed.
        // `FailingCountTotalDenouncementsRepo` panics on `enqueue_action`, so a
        // missing `?` would cause the test to panic rather than fail with an error.
        let svc = DefaultTrustService::new(
            Arc::new(FailingCountTotalDenouncementsRepo),
            Arc::new(PanicReputationRepo),
        );
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
