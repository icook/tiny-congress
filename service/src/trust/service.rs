...
mod tests {
    ...
    /// Stub [`TrustRepo`] that fails `count_daily_actions` with a database error —
    /// used to verify that `revoke_endorsement` propagates the error rather than
    /// silently proceeding to enqueue the action.
    struct FailingCountDailyActionsRepo;

    #[async_trait]
    impl TrustRepo for FailingCountDailyActionsRepo {
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
    ...
    async fn revoke_endorsement_propagates_repo_error_from_count_daily_actions() {
        // `count_daily_actions` is the first repo call in `revoke_endorsement`
        // (after the pure self-action check). If the database fails there, the
        // service must surface the error rather than silently proceeding to enqueue.
        // `FailingCountDailyActionsRepo` panics on `enqueue_action`, so a missing `?`
        // would cause the test to panic rather than return an error.
        let svc = DefaultTrustService::new(
            Arc::new(FailingCountDailyActionsRepo),
            Arc::new(PanicReputationRepo),
        );
        let a = Uuid::new_v4();
        let b = Uuid::new_v4();
