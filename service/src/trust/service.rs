...
mod tests {
    ...
    /// Stub [`TrustRepo`] that passes all pre-enqueue guards in
    /// [`DefaultTrustService::endorse`] (denouncement conflict check, daily quota
    /// check) but returns a database error from `enqueue_action`. Used to verify
    /// the service propagates the error rather than silently succeeding.
    struct FailingEnqueueForEndorseRepo;

    #[async_trait]
    impl TrustRepo for FailingEnqueueForEndorseRepo {
        async fn has_active_denouncement(&self, _: Uuid, _: Uuid) -> Result<bool, TrustRepoError> {
            Ok(false) // no conflict — denouncement guard passes
        }
        async fn count_daily_actions(&self, _: Uuid) -> Result<i64, TrustRepoError> {
            Ok(0) // below quota — quota guard passes
        }
        async fn enqueue_action(
            &self,
            _: Uuid,
    ...
    async fn endorse_propagates_repo_error_from_enqueue_action() {
        ...
        // (self-action check, weight check, daily quota, denouncement conflict,
        // verifier check, slot count). If the database fails at the enqueue step,
        // the service must surface the error rather than silently succeeding.
