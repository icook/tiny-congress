...
mod tests {
    ...
    #[test]
    fn parse_endorse_payload_errors_when_weight_overflows_f32() {
        // A finite f64 value larger than f32::MAX (~3.4e38) overflows to
        // f32::INFINITY when cast with `as f32`. The `#[allow(cast_possible_truncation)]`
        // annotation on that cast is safe only because is_valid_endorsement_weight
        // subsequently rejects infinities. This test verifies that safety net.
        let subject_id = Uuid::new_v4();
        let payload = json!({
            "subject_id": subject_id.to_string(),
            "weight": 1e39_f64,
        ...
        assert!(
            matches!(err, TrustActionError::InvalidPayload(ref msg) if msg.contains("out of range")),
            "weight overflowing to f32::INFINITY must be rejected as out of range, got: {err}"
        );
    }
