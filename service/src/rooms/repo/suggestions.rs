    ...
    E: sqlx::Executor<'e, Database = sqlx::Postgres>,
{
    let row = sqlx::query_as::<_, SuggestionRow>(
        ...
        INSERT INTO rooms__research_suggestions
            (room_id, account_id, suggestion_text, status, filter_reason)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id, room_id, account_id, suggestion_text, status, filter_reason,
...
/// Returns suggestions for a room, newest first.
///
/// # Errors
///
/// Returns `Database` on connection failure.
    ...
    E: sqlx::Executor<'e, Database = sqlx::Postgres>,
{
    let rows = sqlx::query_as::<_, SuggestionRow>(
        r"
        SELECT id, room_id, account_id, suggestion_text, status, filter_reason,
               evidence_ids, created_at, processed_at
